...
export default {
  ...
  methods: {
    ...
    async editCadeau(id) {
      ...
      this.cadeau = { ...cadeau };
      this.imagedata = cadeau.gift_photo;
      this.showForm = true;
    },
    async updateCadeau() {
      const db = getFirestore();
...
<template>
  <main v-if="connected">
    ...
    <div class="wrapper">
      <transition>
        <form enctype="multipart/from-data" @submit.prevent="actionGift" v-if="showForm">
          <div class="card bg-dark">
            <div class="card-header">
              <h5 v-if="mode === 'create'">Ajouter un cadeau</h5>
              <h5 v-if="mode === 'edit'">Modifier un cadeau</h5>
            ...
            <div class="card-footer wrapper endflex">
              <button v-if="mode === 'create'" v-on:click="showSuccessMessage = true" type='submit'
                class="float-left btn btn">Créer</button>
              <button v-if="mode === 'edit'" v-on:click="showSuccessMessage = true" type='submit'
                class="float-left btn btn">modifier</button>
            </div>
          </div>
        </form>
      </transition>
